...
function fetchCodeCompletionTexts(prompt, fileName, MODEL_NAME, API_KEY, USE_GPU) {
  ...
  return new Promise((resolve, reject) => {
    console.log(prompt);
    return (0, import_node_fetch.default)(API_URL, {
      method: "post",
      headers: {
...
var debouncedFetchCodeCompletionTexts = (0, import_lodash.debounce)(fetchCodeCompletionTexts, 2e3);
function activate(context) {
  ...
  const provider = {
    provideInlineCompletionItems: (document, position, context2, token) => __async(this, null, function* () {
      ...
      if (config_default.SEARCH_PHARSE_END.includes(textBeforeCursor[textBeforeCursor.length - 1])) {
        let rs;
        try {
          rs = yield fetchCodeCompletionTexts(last_3, document.fileName, MODEL_NAME, API_KEY, USE_GPU);
        } catch (err) {
